...
│                                   ▲                                  │
│  structured panel ──/ws/agent,/ws/codex──> Claude SDK / Codex app-server │
└─────────────────────────────────────────────────────────────────────┘
```
...
- **Agent CLI**: Claude Code / Codex, spawned on demand by their structured panel bridges; the app probes the user's global installation, **not bundled with the package** (see [§8](#8-built-in-agents)).
...
| `agent.ts` / `codex-agent.ts` / `terminal.ts` / `claude-settings.ts` | Structured panel bridges / CLI probe registration / space-level Claude settings seeding | §8 |
...
| Trigger | Description |
|-|-|
| Open / switch space | bind → snapshot import → syncIndex, serial; Electron startup that restores the last current space also fires a background sync after the renderer rehydrates |
...
StashBase ships with the MCP server binary. The UI's "Connect to AI clients" section does **one-click writes for the three clients with a stable global config path** (claude-code → `~/.claude.json`, codex-cli → `~/.codex/config.toml`, claude-desktop); for the other clients (Cursor / Gemini / VS Code / Cline etc.) it gives a **copyable config snippet** for the user to paste themselves—each client's config format differs (JSON / TOML), and the UI generates by type. After configuring, the external client can access the KB even outside the app. The status API reports separate facts instead of one vague "connected" bit: whether the client config contains StashBase (`configured`) and whether the related CLI binary is on PATH for built-in chat launchers (`cliInstalled`). The renderer maps those into `Configured` / `Not configured` / `CLI missing` pills, and marks `Restart client` immediately after a one-click write until the panel is refreshed.

---

# 8. Built-in Agents

> The builtin agent binds to the current space, saving the steps of opening a terminal, `cd`, and configuring MCP. Claude Code and Codex both render through the same structured panel shell: message bubbles, streaming thinking, expandable tool calls, inline diff approve/reject, history, attachments, and shared composer controls. Their bridges differ behind that shell: Claude uses the Claude Agent SDK, Codex uses Codex app-server.
>
> The two are opened by two launcher buttons in the top right respectively.

## 8.1 Agent CLI probing and registration

The top-right chrome is **one branded icon button per agent** (Claude / Codex, imitating VSCode): clicking one opens the chat panel and **opens a new window (tab) of that agent**, and repeated clicks accumulate multiple parallel sessions (tab titles uniformly `Untitled`, repeats appended with ` 2` / ` 3`). The panel has no separate hide toggle—**closing the last tab auto-collapses the panel** (the reducer `CHAT_TAB_CLOSE` sets `chatOpen` to false). The entry for creating a new tab is the chrome launcher (the tab bar itself only shows already-open tabs); the structured panel header additionally has a same-agent "new conversation" `+`. **Each tab locks to its own agent**.

Renderer-facing agent metadata lives in [agentCatalog.tsx](../web-src/src/agentCatalog.tsx), which is the single registry for each agent's label, icon, WebSocket endpoint, capability flags, and MCP client id. The top-right launchers, tab glyphs, AgentView, and Settings → MCP client rows all read that same catalog so UI chrome and connection status stay aligned while backend-specific protocol differences remain behind the bridge.
...
Claude sessions are stored in Claude Code's own standard location (`~/.claude/`), so **a session started by the StashBase panel and an external terminal / claude.ai are fully interoperable**—start on one side, resume on the other, no extra mechanism needed. Codex sessions are backed by Codex app-server thread APIs and are exposed to the same History dropdown shape through [routes/codex-sessions.ts](../server/routes/codex-sessions.ts).

## 8.4 Structured panel

Aligned with the VSCode Claude extension's structured panel: render the agent output into message bubbles, expandable tool calls, inline diff (approve / reject), and thinking blocks. The renderer's [AgentView.tsx](../web-src/src/components/AgentView.tsx) is shared by both agents; capability flags decide which controls appear, and the Controls menu header carries a short agent-specific note so Claude's live permission modes and Codex's fixed approval/sandbox behavior are not visually indistinguishable. Claude exposes permission modes plus effort; Codex currently exposes effort while approval policy and sandbox are fixed at thread start by the Codex bridge.

Data flow:

```text
AgentView (web-src) ──/ws/agent──> server/agent.ts ──Claude Agent SDK──> claude
AgentView (web-src) ──/ws/codex──> server/codex-agent.ts ──Codex app-server──> codex
  structured events <───── normalized wire protocol <───── SDK / app-server stream
```

- **How it runs**: [server/agent.ts](../server/agent.ts)'s `AgentSession` uses `@anthropic-ai/claude-agent-sdk`'s `query()`, streaming-input mode (a `Pushable` queue feeds user messages, the session lives long). [server/codex-agent.ts](../server/codex-agent.ts)'s `CodexSession` starts `codex app-server --listen stdio://`, opens or resumes a thread, and relays turn/item notifications into the same renderer event model. cwd = current space for both.
- **wire protocol** (JSON over `/ws/agent` and `/ws/codex`): server→client are normalized events `text` / `thinking` (streaming deltas), `tool` / `tool-result` (with tool_use_id reconciliation), `permission`, `session-id` (for History to mark the current session), `turn-end` / `error` / `exit`; client→server are `prompt` / `permission-reply` / `set-mode` / `interrupt` / `close`. SDK/app-server native event shapes are normalized away on the server, and the renderer does not touch provider-specific stream types.
- **Permission round-trip**: Claude uses the SDK's `canUseTool` callback: read-class tools (Read / Grep / search…) are passed directly; write / execute class (Edit / Write / Bash / MCP write) round-trip to the client, suspending a Promise to wait for the user to approve/reject—this is the foundation of inline-diff approval. Codex maps app-server approval requests (`commandExecution`, `fileChange`, `permissions`) into the same renderer `permission` event shape. **The permission mode** is Claude-only: the composer's Controls menu sends `set-mode` → `query.setPermissionMode()` for `default` / `acceptEdits` / `plan` / `auto`. Codex ignores live `set-mode` because its approval policy and sandbox are provided at thread/turn setup. **Effort (thinking depth)** is shared and goes through the connection URL's `?effort=` parameter (`low…max`, default `high`); changing the level relies on reconnection for an empty session and otherwise takes effect on the next new session.
- **Config loading**: Claude uses `settingSources: ['user','project','local']` so the panel sees the user's global + that space's CLAUDE.md / skills / MCP (including the KB MCP in `~/.claude.json`). Codex reads its standard config via the app-server process (including the KB MCP in `~/.codex/config.toml`). Both bridges clear `ELECTRON_RUN_AS_NODE` and pass `STASHBASE_WINDOW_ID`.
- **StashBase directional preamble**: relying on client config alone leaves the model with a cwd but no guarantee it knows it is inside StashBase, what `search_kb`/`reindex` are for, or what the rulebook says. Both bridges inject `buildStashbasePreamble(cwd)` deterministically: Claude appends it to the `claude_code` preset system prompt; Codex passes it as `developerInstructions` on thread start/resume. The preamble is generated fresh per session (cwd is fixed within the session lifecycle). "Proactively query the KB" is **soft-recommendation** wording rather than a hard requirement—preserving the [§6.1](#61-call-entry) principle "when to use MCP / shell is decided by the agent itself".
- **Authentication**: each bridge relies on the user's normal CLI login/session. Claude reads the credentials the user's `claude` login wrote down (Keychain / `~/.claude`), and a subscription (Pro/Max) is used directly without an API key—do not inject `ANTHROPIC_API_KEY` into env (its priority would override the subscription). Codex likewise runs the user's installed `codex` CLI/app-server rather than a bundled credential flow.
...
- **History / resume**: the header History dropdown uses one renderer component but routes by agent. Claude session management goes through [routes/sessions.ts](../server/routes/sessions.ts) (`/api/agent/sessions` list, `/:id/messages` get transcript, `PATCH` rename, `DELETE` delete), backed by the SDK's on-disk session store (`~/.claude/projects/`). Codex session management goes through [routes/codex-sessions.ts](../server/routes/codex-sessions.ts), backed by Codex app-server structured thread APIs. Opening a session first maps the transcript into blocks isomorphic to the wire protocol (historical tools are always `done`/`error`) and renders them out, then reconnects with `?resume=<id>` so the backend appends to the same session/thread. **Resume runs under the current space's cwd** (the panel is always bound to the current space), and recovering an old session across projects only loads its conversation history, continuing in the current space. `resume` and `effort` are consumed only once on the connection URL and cleared after connecting, avoiding a mistaken resume on subsequent reconnections.
...
On the renderer side, [AgentView.tsx](../web-src/src/components/AgentView.tsx) maintains an ordered block array (user / assistant / thinking / tool / error), incrementally updated by normalized events; the diff is computed client-side from Claude Edit/Write/MultiEdit inputs and Codex `fileChange` approval payloads (unified patches when present, otherwise before/after or create/delete content). It trims the common head and tail + a middle ±, not a full LCS.
