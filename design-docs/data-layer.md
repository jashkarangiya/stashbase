...
Scheduler priority is an optimization, not a source of truth. Tasks are deduplicated by canonical absolute POSIX source path. The filesystem-path module owns comparison identity and subtree semantics: Windows folds separators and path case without changing retained source spelling, while POSIX remains case-sensitive. Each lane orders queued work by interactive/open-folder/background urgency, then format cost, then enqueue order. After 60 seconds, background work may age to open-folder urgency but never above interactive work. Manual reprocess is interactive; already-running tasks are non-preemptive.
...
Renames and moves use absolute source path identity. `server/filesystem-path.ts` owns the distinction between canonical source spelling and comparison identity. On Windows it restores the stored member-root spelling and actual on-disk component spelling before derived hashes, failure rows, or index identities are consulted; path comparisons remain case-insensitive. Its root-aware operations cover filesystem, drive, and UNC roots, and its existing/creatable resolution modes apply the same symlink-escape contract to normal file operations, upload, and explicit-folder preparation. A file rename request with a basename target stays in the source file's current parent folder; requests with a folder-relative target path are moves. Structured text files can move index rows when the content remains readable. PDF/image/DOCX moves clear old derived artifacts and old index rows, then queue conversion again under the new absolute source path.
...
- `server/filesystem-path.ts` is the single path-semantics seam. It produces canonical absolute source spelling and comparison identity, handles POSIX/drive/UNC root operations, restores existing Windows component spelling, rejects ambiguous Windows device/drive-relative input, and applies lexical/existing/creatable symlink-safe resolution (`server/filesystem-path.ts:20-159`, `server/filesystem-path.ts:170-210`). Folder membership retains stored spelling while using that seam for exact and longest-prefix lookup (`server/folder.ts:105-130`). `files.ts`, upload, and explicit-folder preparation delegate their filesystem containment checks to the same resolver (`server/files.ts:85-90`, `server/routes/upload.ts:110-133`, `server/routes/indexing.ts:76-106`).
- Canonical absolute source path is the conversion task's retained spelling; its map identity and subtree operations come from the filesystem-path module (`server/conversion-scheduler.ts:105-218`, `server/conversion-scheduler.ts:246-257`, `server/conversion-scheduler.ts:487-489`). Duplicate schedules share one completion promise and may only raise urgency or lower cost while queued. Effective ordering is urgency, then cost, then enqueue sequence (`server/conversion-scheduler.ts:319-337`). Ageing transitions bump the lane revision and affected per-file versions (`server/conversion-scheduler.ts:441-484`). In-flight progress and durable failure lookup/cleanup use the same identity semantics while durable rows preserve source spelling (`server/conversion-status.ts:41-107`, `server/state-db.ts:315-369`).
